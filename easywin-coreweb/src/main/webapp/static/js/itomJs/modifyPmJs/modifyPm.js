...
$(function(){
	...
	initModifyPmTable();
		...
		//出差管理
	 	function initModifyPmTable(morePageNum){
			if(morePageNum){
				pageNum = morePageNum;
			}else{
				pageNum = 0;
			...
			$.each($("#formTempData").find("select"),function(index,obj){
				...
				if(options && options.length>0){
	     		...
	     		//取得数据
	     		getSelfJSON("/iTOm/modifyPm/ajaxListPagedModifyPm",params,function(data){
	     			if(data.status=='y'){
        ...
        function PageCallback(index, jq) {  
		 ...
		 function constrDataTable(modifyPms){
			 if(modifyPms && modifyPms[0]){
				 var rowNumIndex = 1;
				 $.each(modifyPms,function(objIndex,obj){
					 //一行数据
					 var tr = $("<tr></tr>");
					 $(tr).data("obj",obj);
					 ...
					 $(tr).append(tdUser);
					 //事件单流水号
					 var tdModifyId = $(tdMod).clone();
					 $(tdModifyId).html(obj.modifyId);
					 $(tr).append(tdModifyId);
					 ...
					 //事件性质
					 var tdModifyAttr = $(tdMod).clone();
					 $(tdModifyAttr).html(obj.modifySource);
					 $(tr).append(tdModifyAttr);
					 ...
					 //所属系统类型
					 var tdModifyType = $(tdMod).clone();
					 $(tdModifyType).html(obj.modifyType);
					 $(tr).append(tdModifyType);
					 
					 //所属系统类型
					 var tdModifyStatus = $(tdMod).clone();
					 $(tdModifyStatus).html(obj.modifyStatus);
					 $(tr).append(tdModifyStatus);
