...
$(function(){
	...
	initReleasePmTable();
		...
		//出差管理
	 	function initReleasePmTable(morePageNum){
			if(morePageNum){
				pageNum = morePageNum;
			}else{
				pageNum = 0;
			...
			$.each($("#formTempData").find("select"),function(index,obj){
				...
				if(options && options.length>0){
	     		...
	     		//取得数据
	     		getSelfJSON("/iTOm/releasePm/ajaxListPagedReleasePm",params,function(data){
	     			if(data.status=='y'){
        ...
        function PageCallback(index, jq) {  
		 ...
		 function constrDataTable(releasePms){
			 if(releasePms && releasePms[0]){
				 var rowNumIndex = 1;
				 $.each(releasePms,function(objIndex,obj){
					 //一行数据
					 var tr = $("<tr></tr>");
					 $(tr).data("obj",obj);
					 ...
					 $(tr).append(tdUser);
					 //事件单流水号
					 var tdReleaseId = $(tdMod).clone();
					 $(tdReleaseId).html(obj.releaseId);
					 $(tr).append(tdReleaseId);
					 ...
					 //事件性质
					 var tdReleaseAttr = $(tdMod).clone();
					 $(tdReleaseAttr).html(obj.releaseSource);
					 $(tr).append(tdReleaseAttr);
					 ...
					 //所属系统类型
					 var tdReleaseType = $(tdMod).clone();
					 $(tdReleaseType).html(obj.releaseType);
					 $(tr).append(tdReleaseType);
					 
					 //所属系统类型
					 var tdReleaseStatus = $(tdMod).clone();
					 $(tdReleaseStatus).html(obj.releaseStatus);
					 $(tr).append(tdReleaseStatus);
