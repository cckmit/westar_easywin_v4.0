...
$(function(){
	...
	initEventPmTable();
		...
		//出差管理
	 	function initEventPmTable(morePageNum){
			if(morePageNum){
				pageNum = morePageNum;
			}else{
				pageNum = 0;
			...
			$.each($("#formTempData").find("select"),function(index,obj){
     		...
     		//取得数据
     		getSelfJSON("/iTOm/eventPm/ajaxListPagedEventPm",params,function(data){
     			if(data.status=='y'){
        ...
        function PageCallback(index, jq) {  
		 ...
		 function constrDataTable(eventPms){
			 if(eventPms && eventPms[0]){
				 var rowNumIndex = 1;
				 $.each(eventPms,function(objIndex,obj){
					 //一行数据
					 var tr = $("<tr></tr>");
					 $(tr).data("obj",obj);
					 ...
					 $(tr).append(tdUser);
					 //事件单流水号
					 var tdEventId = $(tdMod).clone();
					 $(tdEventId).html(obj.eventId);
					 $(tr).append(tdEventId);
					 ...
					 //事件性质
					 var tdEventAttr = $(tdMod).clone();
					 $(tdEventAttr).html(obj.eventAttr);
					 $(tr).append(tdEventAttr);
					 
					 //事件优先级
					 var tdEventPriDegree = $(tdMod).clone();
					 $(tdEventPriDegree).html(obj.eventPriorityDegree);
					 $(tr).append(tdEventPriDegree);
					 ...
					 //事件状态
					 var tdEventStatus = $(tdMod).clone();
					 $(tdEventStatus).html(obj.eventStatus);
					 $(tr).append(tdEventStatus);
