$(function(){
    ...
    $("body").on("click","#forceInBtn",function(){
        forceIn(sid,$(this),'080')
    })
});
function searchItem(index,userId){
    //启动加载页面效果
    layer.load(0, {shade: [0.6,"#fff"]});
    var url ="/product/listPagedPro?sid="+sid+"&pager.pageSize=12&searchTab="+index+"&t="+Math.random();;
    if(index == 11){
        url = url + "&ownerType=1";
    }else if(index == 12){
        url = url + "&attention=1"
    }else if(index == 13){
        url = url + "&principal=" + userId;
    }else if(index == 14){
        url = url + "&manager=" + userId;
    }
...
function settingItem(index,ts){
    ...
    if(index==32){//操作权限
        modOperateConfig(sid,'080',ts,preActive)
    }
}
//添加产品
function addPro(){
    var url = "/product/addProPage?sid="+sid;
    openWinByRight(url);
}
//查看客户
function viewItem(itemId){
    viewModInfo(itemId,'080',-1);
}
...
$(function(){
    $("#batchDelItem").click(function(){
        if(checkCkBoxStatus('ids')){
            window.top.layer.confirm('确定要删除该产品吗？',{title:false,closeBtn:0,icon:3}, function(){
            ...
            window.top.layer.msg('请先选择需要删除的产品。',{title:false,closeBtn:0,icon:7});
        }
    });
    //批量移交
...
function itemCompress(sid){
    ...
    if(ckBoxs.length==0){
        layer.alert('请先选择需要合并的产品。');
    }else if(ckBoxs.length==1){
        layer.alert('请先选择至少两个产品');
    }else if(ckBoxs.length>3){
        layer.alert('最多选择三个产品');
    }else{
        var ids = new Array();
...
function getItemHtml(itemVo){
    ...
    html +='\n 			<a href="javascript:void(0)" class="project-box '+readState+'" onclick="readMod(this,\'item\','+itemVo.id+',\'080\');viewItem('+itemVo.id+');">';
    ...
    html +='\n'+itemVo.ownerName;
    html +='\n 			</span>';
    html +='\n			<a href="javascript:void(0)" class="link pull-right margin-left-10" attentionState="'+itemVo.attentionState+'" busType="080" busId="'+itemVo.id+'" describe="0" iconSize="sizeMd" onclick="changeAtten(\''+sid+'\',this)">';
...
//进行中状态筛选
function stateFilter(stateId,ts){
