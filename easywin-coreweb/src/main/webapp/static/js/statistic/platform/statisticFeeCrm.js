...
$(function(){
	...
	$("body").on("click","a[data-crmfee-month-user]",function(){
		var month = $(this).attr("data-crmfee-month-user");
		...
		url = url+"&listCreator[0].id="+userId
		url = url+"&listCreator[0].userName="+encodeURIComponent(userName);
...
var monThObj = {};
//出差管理
function initStatisticFeeCrmTable(morePageNum){
	if(morePageNum){
		pageNum = morePageNum;
	}else{
		loadDone=0;
	...
	$.each($("#formTempData").find("select"),function(index,obj){
		...
		if(options && options.length>0){
 		...
 		//取得数据
 		getSelfJSON("/statistics/platform/ajaxListStatisticFeeCrm",params,function(data){
 			if(data.status=='y'){
 				...
 				var nowMonth = data.nowMonth;
 	           	if(recordList && recordList[0]){
               		...
               		initStatisticFeeCrmTable(++pageNum);
               		return;
               	 }else{
               		loadDone = 1;
               		constrAll(nowMonth);
               		if(!morePageNum){ 
               			...
               			$("#editabledatatable").parent().height();
 ...
 function constrDataTable(statisticCrmVos,nowMonth){
	 if(statisticCrmVos && statisticCrmVos.length>0){
		 ...
		 $.each(statisticCrmVos,function(objIndex,obj){
			 ...
			 //姓名
			 var tdUserName = $(tdMod).clone();
			 $(tdUserName).html(obj.userName);
			 $(tdUserName).css("width","90px");
			 ...
			 for(var i=1;i<13;i++){
				 ...
				 if(myDate>actDate){
					 ...
					 if(itemFee && itemFee>0){
						 ...
						 $(itemFeeA).attr("data-crmfee-month-user",i);
