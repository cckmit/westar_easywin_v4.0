...
var FolderObj = {
		initEvent:function(){
			...
			//通过ajax获取部门树
			$.post('/fileCenter/ajaxListTreeFolder', {random : Math.random(),sid:EasyWin.sid,id:classifyId}, function(tree) {
				//创建一个默认的根节点
			 ...
			 if (ckBoxs != null) {
				 $.each(ckBoxs,function(index,ckBox){
					 if ($(this).attr('checked')) {
						 var classify = $(this).parents('tr').data("classify");
						 var ownState = (classify.userId==EasyWin.userInfo.userId);
						 if(ownState){
							 dirs.push(classify);
						 }
		...
		editDirPre:function(ts,calback){//编辑文件夹
			...
			var dirName = $(ts).parents('tr').find(".dirName").find("input").val().replace(/\s+/g,"");
		 	if(!dirName){
		 		...
		 		var pubState = $(ts).parents('tr').find("#pubState").val();
		 		var parentId = $("#classifyId").val();
		 		if(parentId && parentId>0){
		 			var node = zTreeObj.getNodeByParam("id",parentId);
		 			if(node.pubState==0 && pubState==1){//私有的
		 				layer.tips("私有文件夹不能创建公开文件夹！",$(ts).parents('tr').find("#pubState"),{tips:1});
		 	 			return;
		 			}
		 		}
		 		//默认能够修改
...
var FileObj = {
		initEvent:function(){
			...
			$("body").on("click",".moveFile",function(){
				...
				FileObj.Opt.moveFile(0,$("#classifyId").val(),function(parentId){
					...
					var params = {
						...
						fileDescribe:fileDetail.fileDescribe
					}
					postUrl(url,params,function(data){
						if(data.status=='y'){
			...
			$("body").on("click",".viewOfficeFile",function(){
				var fileDetailObj = $(this).parents('tr').data("fileDetail");
				viewOfficePage(fileDetailObj.fileId,fileDetailObj.uuid,fileDetailObj.fileName,fileDetailObj.fileExt,EasyWin.sid,fileDetailObj.moduleType,fileDetailObj.id);
			});
			//查看图片
			$("body").on("click",".viewPicFile",function(){
				var fileDetailObj = $(this).parents('tr').data("fileDetail");
				showPic('/downLoad/down/'+fileDetailObj.uuid+'/'+fileDetailObj.fileName,EasyWin.sid,fileDetailObj.fileId,fileDetailObj.moduleType,fileDetailObj.id)
			});
			//下载
			$("body").on("click",".dowmLoadFile",function(){
				var fileDetailObj = $(this).parents('tr').data("fileDetail");
				var url = "/downLoad/down/"+fileDetailObj.uuid+"/"+fileDetailObj.fileName+"?sid="+EasyWin.sid+"&addLogstate=1";
			...
			var url = "/fileCenter/ajaxListPagedFolderAndFile?sid="+EasyWin.sid;
			postUrl(url,params,function(data){
				loadDone = 1;
				if(pageNum == 0){
			...
			$.each(files,function(index,fileDetailObj){
				var fileTr = $('<tr class="fileInfoOne"></tr>');
				$(fileTr).data("fileDetail",fileDetailObj);
				//序号
				var indexTd = $('<td height="40"></td>');
				var indexLable = $('<label></label>')
				...
				$(fileNameA).html(fileDetailObj.fileName);
				$(fileNameTd).append(fileNameA);
				$(fileTr).append($(fileNameTd));
				//文件描述
				var fileFromTd = $('<td height="40"></td>');
				fileFrom = fileDetailObj.fileDescribe?fileDetailObj.fileDescribe:"";
				$(fileFromTd).html(fileFrom);
				$(fileTr).append($(fileFromTd));
				
				//文件大小
				var fileSizeTd = $('<td height="40"></td>');
				$(fileSizeTd).html(fileDetailObj.sizem);
				$(fileTr).append($(fileSizeTd));
				//文件后缀
				var fileExtTd = $('<td height="40"></td>');
				$(fileExtTd).html(fileDetailObj.fileExt);
				$(fileTr).append($(fileExtTd));
				
				//上传人员
				var userTd = $('<td height="40" class="infoDetail"></td>');
				var userDiv = $('<div class="ticket-user pull-left other-user-box"></div>');
				var userImg = $('<img src="/downLoad/userImg/'+EasyWin.userInfo.comId+'/'+fileDetailObj.userId+'" class="user-avatar" />');
				var userName = $('<span class="user-name"></span>');
				$(userName).html(fileDetailObj.userName);
				$(userDiv).append($(userImg));
				$(userDiv).append($(userName));
				...
				$(dateTimeTd).html(fileDetailObj.recordCreateTime.substring(0,10));
				$(fileTr).append($(dateTimeTd));
				
				var optTd = FileObj.constrOptTd(fileDetailObj);
				$(optTd).css("padding",'2px 2px');
				$(fileTr).append($(optTd));
			...
			setStyle();
		},constrOptTd:function(fileDetailObj){
			var optTd = $('<td colspan="2" style="display: none;text-align:center;" class="opt"></td>');
			
			var modA = $('<a href="javascript:void(0)" class="btn btn-info ws-btnBlue btn5 margin-left-5" style="color:#fff"></a>');
			var canMove = (fileDetailObj.userId == EasyWin.userInfo.userId)
			if(canMove){
				var moveA = $(modA).clone();
				$(moveA).html('移动');
			...
			var canShare = (fileDetailObj.userId == EasyWin.userInfo.userId);
			if(canShare){
			...
			var canDel = (fileDetailObj.userId == EasyWin.userInfo.userId && fileDetailObj.moduleType=='013');
			if(canDel){
				var delA = $(modA).clone();
				$(delA).html('删除');
			...
			var canViewOffice = (fileDetailObj.fileExt=='doc' 
								|| fileDetailObj.fileExt=='docx' 
								|| fileDetailObj.fileExt=='xls' 
								|| fileDetailObj.fileExt=='xlsx' 
								|| fileDetailObj.fileExt=='ppt' 
								|| fileDetailObj.fileExt=='pptx'
								|| fileDetailObj.fileExt=='txt' 
								|| fileDetailObj.fileExt=='pdf');
			if(canViewOffice){
			...
			var canViewPic = (fileDetailObj.fileExt=='jpg'
							||fileDetailObj.fileExt=='bmp'
							||fileDetailObj.fileExt=='gif'
							||fileDetailObj.fileExt=='jpeg'
							||fileDetailObj.fileExt=='png');
			if(canViewPic){
			...
			var doanLoadA = $(modA).clone();
			$(doanLoadA).html('下载');
			$(doanLoadA).addClass('dowmLoadFile');
			$(optTd).append($(doanLoadA));
...
FileObj.Opt = {
		moveFile:function(preDirs,oldPId,callback){//移动文件
			 window.top.layer.open({
				 type: 2,
				  ...
				  maxmin: false,
				  move: false,
				  content: '/common/dirOnePage?sid=' + EasyWin.sid+'&t='+Math.random(),
				  btn: ['确定', '取消'],
		...
		},shareFile:function(fileDetail,callback){
			var options = {
					 type: 2,
					  title: false,
					  closeBtn:0,
					 ...
					 success: function(layero,index){
						  ...
						  iframeWin.initData(id,'013');
					  }
				 };
			var canShare = (fileDetail && fileDetail.userId != EasyWin.userInfo.userId);
...
var FileCenter = {
		initEvent:function(){
			...
			$("body").on("click","[data-optType='del']",function(){
				...
				if((dirs.length+files.length)==0){
					window.top.layer.alert("请确认来源是文档模块,且是自己上传或创建的！");
					return;
				}else{
			...
			$("body").on("click","[data-btn='addFiles']",function(){
				...
				FileCenter.addFiles(classifyId,function(){
					 //重新加载文件数据
					 FileObj.initData(0);
					 showNotification(1, "操作成功");
				});
			...
			FileObj.Opt.moveFile(preDirs, $("#classifyId").val(), function(parentId){
				var url = "/fileCenter/batchMove?sid="+sid+"&t="+Math.random();
				var params = {
		...
		},addFiles:function(classifyId,callback){
			var url = "/fileCenter/addFilePage?sid="+EasyWin.sid+'&classifyId=-1&t='+Math.random();
