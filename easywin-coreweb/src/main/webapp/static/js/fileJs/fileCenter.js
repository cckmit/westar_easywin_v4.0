...
//上传新文件
 function addFiles(sid){
	var url = "/fileCenter/addFilePage?sid="+sid+"&classifyId="+$("#classifyId").val()+'&t='+Math.random();
...
//保存文件夹
 function saveDir(ts,sid){
 	...
 	if(dirName.length==0){
 		...
 		$.ajax({
 	        type:"post",
 	        url:"/fileCenter/addDir?sid="+sid+"&t="+Math.random(),
 	        dataType: "json", 
 	        beforeSend:function(){
 ...
 */
 ...
 function moveDir(ts,id,sid){
	 ...
	 window.top.layer.open({
		 type: 2,
		  ...
		  maxmin: false,
		  move: false,
		  content: '/common/dirOnePage?sid=' + sid+'&t='+Math.random(),
		  btn: ['确定', '取消'],
		  yes: function(index, layero){
 ...
 function shareFile(ts,id,fileId,sid){
	 window.top.layer.open({
		 type: 2,
		  ...
		  maxmin: false,
		  move: false,
		  content:'/common/shareGroupPage?sid=' + sid+'&id='+id+'&busType=013&t='+Math.random(),
		  btn: ['确定', '取消'],
		  yes: function(index, layero){
			  var iframeWin = window.top.window[layero.find('iframe')[0]['name']];
  ...
  * @return
  */
 function checkAllPre(ckBoxElement, ckBoxName) {
 ...
 */
		 ...
		 if (ckBoxs != null) {
			 for ( var i = 0; i < ckBoxs.length; i++) {
				 if ($(ckBoxs[i]).attr('checked') && $(ckBoxs[i]).attr('owner')=='true') {
					 ...
					 strs=$(ckBoxs[i]).val().split("@"); //字符分割 
					 if(strs[1].indexOf('013')==0){
						 files.push($(ckBoxs[i]).val());
					 }else{
		  ...
		  yes: function(index, layero){
			  ...
			  if(parentId){
				  ...
				  $("#listFileForm").ajaxSubmit({
						...
						error:function(XmlHttpRequest,textStatus,errorThrown){
							showNotification(1,"系统错误请联系管理人员！")
						}
					});
			  }
		  }
 ...
 function batchShare(sid,files){
	 ...
	 window.top.layer.open({
		 type: 2,
		  ...
		  maxmin: false,
		  move: false,
		  content:'/common/shareGroupPage?sid=' + sid+'&id=0&busType=013&t='+Math.random(),
		  btn: ['确定', '取消'],
		  yes: function(index, layero){
			  var iframeWin = window.top.window[layero.find('iframe')[0]['name']];
			  var result = iframeWin.returnScope();
  ...
  * 
  * @param id 模块主键
  * @param modType  1周报 2 任务 3项目 4客户 5 知识 6问 7答
  * @param sid
  * @return
  */
 function viewDetail(id,modType,sid){
	 var url;
	 if(0==modType.indexOf('006')){
		 url='/weekReport/viewWeekReport?id='+id+'&sid='+sid;
	 }else if(0==modType.indexOf('003')){
		 url='/task/viewTask?id='+id+'&sid='+sid;
	 }else if(0==modType.indexOf('050')){
         url='/daily/viewDaily?id='+id+'&sid='+sid;
     }else if(0==modType.indexOf('005')){
		 url='/item/viewItemPage?id='+id+'&sid='+sid;
	 }else if(0==modType.indexOf('080')){
         url='/product/viewProPage?id='+id+'&sid='+sid;
     }else if(0==modType.indexOf('012')){
		 url='/crm/viewCustomer?id='+id+'&sid='+sid;
	 }else if(0==modType.indexOf('013')){
	 }else if(0==modType.indexOf('011')){
		 url='/qas/viewQuesPage?id='+id+'&sid='+sid;
	 }else if(0==modType.indexOf('004')){
		 url='/vote/voteDetail?id='+id+'&sid='+sid;
	 }
	 if(url){
		 layer.closeAll();
