$(function(){
	...
	$(".operaterReplyTextarea_-1").bind("paste cut keydown keyup focus blur",function(event){
		resizeVoteH('otherinstituAttrIframe');
    });
});
//样式设置
function setStyle(){
 ...
 * talkPId 回复的父节点
 */
function addTalk(instituId,talkPId,ts,ptalker){
	...
	if(""==content){
		...
		//异步提交表单 投票讨论
	    $("#instituForm").ajaxSubmit({
	        type:"post",
	        url:"/institution/addInstituTalk?sid="+EasyWin.sid+"&t="+Math.random(),
	        data:{"busId":instituId,
		        "parentId":talkPId,
		        "content":content,
		        "ptalker":ptalker,
	        ...
	        success:function(data){
	        	var talk = data.talk;
			    var html = getTalkStr(talk,EasyWin.sid,data.userInfo,'040','otherinstituAttrIframe');
			    $(textarea).val('');
		       	if(talkPId==-1){
	...
	window.top.layer.confirm("确定要删除该回复吗?", {
	  ...
	  ,icon:3
		...
		if(isLeaf==1){
		    ...
		    $("#instituForm").ajaxSubmit({
		        type:"post",
		        url:"/institution/delInstituTalk?sid="+EasyWin.sid+"&t="+Math.random(),
