(function(a) {
    ...
    SewisePlayerSkin.BannersAds = function(b, c) {
        ...
        function d() {
            ...
            if (p < u - 1) {
                var b = SewisePlayerSkin.Utils.stringer.hmsToDate(m[p + 1].time).getTime();
                a > b && (p++, j(p))
    ...
    SewisePlayerSkin.ClarityWindow = function(b) {
        ...
        this.initialClarities = function(b) {
            ...
            for (b = 0; b < t; b++) {
                var G;
                f[b].selected ? (G = ' checked = "checked" ', g = b, i.updateClarityBtnText(f[b].name)) : G = " ";
                j += '<input style="width: 20px; height: 18px; " type="radio" name="radio_clarity"' + G + '"value="' + f[b].name + '">' + f[b].name + "\n"
            }
    ...
    SewisePlayerSkin.ControlBar = function(b, c, k, i) {
        function d() { ! 1 == ea && (H.show(), I.css("visibility", "visible"), k.show(), ea = !0, D = setTimeout(f, fa))
        }
        function j() {
        ...
        function F(a) {
            ...
            s.unbind("touchend", F);
            if (1 == e.changedTouches.length) W = e.changedTouches[0].pageX,
            J != W && (q = y.width(), K = q / x, n.seek(K * L));
            P = !1
        }
        function m(a) {
        ...
        function G(a) {
            ...
            r.unbind("touchend", G);
            if (1 == e.changedTouches.length) X = e.changedTouches[0].pageX,
            M != X && (o = w.width(), A = o / v, n.setVolume(A), Y())
        }
        function V() {
            null != document.fullscreenElement || null != document.msFullscreenElement || null != document.mozFullScreenElement || null != document.webkitFullscreenElement ? (console.log("document to fullscreen"), c.fullScreen()) : c.normalScreen();
            B.timeUpdate(C)
        }
        function da() {
            c.resize();
        ...
        this.fullScreen = function() {
            ...
            b.requestFullscreen ? b.requestFullscreen() : b.msRequestFullscreen ? b.msRequestFullscreen() : b.mozRequestFullScreen ? b.mozRequestFullScreen() : b.webkitRequestFullscreen ? b.webkitRequestFullscreen() : Z.webkitEnterFullscreen ? (Z.play(), Z.webkitEnterFullscreen(), aa.show(), S.hide()) : (c.fullScreen("not-support"), B.timeUpdate(C), a(window).bind("resize", ia));
            ha = "full"
        };
    ...
    b(document).ready(function() {
        ...
        SewisePlayerSkin.IVodSkin.volume = function(a) {
            f.initVolume(a)
        };
