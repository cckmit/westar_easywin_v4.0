				...
				<div class="col-md-12 col-xs-12 ">
					<div class="widget">
						<div class="widget-header ps-titleHeader bordered-bottom bordered-themeprimary">
							...
							<div>
							<form action="/mail/listPagedSendMail" class="subform" id="searchForm">
						...
						<c:choose>
							<c:when test="${not empty lists}">
								<div class="widget-body">
											...
											<tbody>
												...
												<c:forEach items="${lists}" var="obj" varStatus="vs">
													<tr class="optTr">
														...
														<td>${obj.account}</td>
														<td>
															<c:forEach items="${obj.listMailAddressee}" var="addr" varStatus="vsa">
																${vsa.count>1?';':''}${addr.personal}${not empty addr.personal?'（':''}${addr.address}${not empty addr.personal?'）':''}
															</c:forEach>
														
														</td>
														<td>${obj.sendTime}</td>
														<td><a href="javascript:void(0);" onclick="sendTurn('${obj.id}');">转发</a></td>
													</tr>
												</c:forEach>
												</c:when>
												<c:otherwise>
									...
									<tags:pageBar url="/mail/listPagedSendMail"></tags:pageBar>
								</div>
							</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		function savePulldata(sid,pullType) {
			window.top.layer.confirm('确定同步邮件到本地？',{icon:3,title:'询问框'}, function(index){
				...
				$.post("/mail/addPulldata?sid=${param.sid}&pullType="+pullType,{Action:"post",folder:'已发送',accountId:accountId,},     
	 				function (msgObjs){
					if(msgObjs=="同步成功"){
