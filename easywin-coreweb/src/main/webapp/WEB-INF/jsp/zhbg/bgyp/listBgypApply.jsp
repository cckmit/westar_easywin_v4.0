			...
			<div class="row">
				<div class="col-md-12 col-xs-12 ">
					<div class="widget">
						...
						<div>
							<form action="/bgypApply/listPagedBgypApply" id="searchForm" class="subform">
								...
								<input type="hidden" name="applyCheckState" value="${bgypApply.applyCheckState }">
								<input type="hidden" name="applyUserId" value="${bgypApply.applyUserId }">
								<input type="hidden" name="purUserName" value="${bgypApply.userName }">
									...
									<div class="table-toolbar ps-margin">
										<div class="btn-group">
											...
											<ul class="dropdown-menu dropdown-default">
												<li><a href="javascript:void(0)" class="clearThisElement" relateElement="purUserId" relateElementName="purUserName">不限条件</a>
												</li>
												<li><a href="javascript:void(0)" class="userOneElementSelect" relateElement="purUserId" relateElementName="purUserName">人员选择</a>
												</li>
											</ul>
										</div>
									</div>
									<div class="table-toolbar ps-margin">
											<div class="btn-group">
												<a class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" title="流程状态筛选">
													<c:choose>
														<c:when test="${not empty bgypApply.applyCheckState}">
															<font style="font-weight:bold;">
																<c:choose>
																	<c:when test="${bgypApply.applyCheckState eq 0 or bgypApply.applyCheckState eq 3}">待送审</c:when>
																	<c:when test="${bgypApply.applyCheckState eq 1}">待审核</c:when>
																	<c:when test="${bgypApply.applyCheckState eq 2}">已入库</c:when>
																</c:choose>
															</font>
														</c:when>
												...
												<ul class="dropdown-menu dropdown-default">
													<li><a href="javascript:void(0)" class="clearThisElement" relateElement="applyCheckState">不限条件</a>
													</li>
													<li><a href="javascript:void(0)" class="setElementValue" relateElement="applyCheckState" dataValue="0">待送审</a>
													</li>
													<li><a href="javascript:void(0)" class="setElementValue" relateElement="applyCheckState" dataValue="1">待审核</a>
													</li>
													<li><a href="javascript:void(0)" class="setElementValue" relateElement="applyCheckState" dataValue="2">已入库</a>
													</li>
												</ul>
											</div>
										</div>
								...
								<div class="ps-margin ps-search searchCond hide">
									...
									<input name="remark" value="${bgypApply.remark}" class="form-control ps-input formElementSearch" type="text" placeholder="请输入关键字">
										<a href="javsscript:void(0)" class="ps-searchBtn"><i class="glyphicon glyphicon-search circular danger"></i> </a>
									</span>
								</div>
						...
						<div class="widget-body">
											...
											<tr class="optTr">
												<td class="optTd" style="height: 47px">
													<label class="optCheckBox" style="display: none;width: 20px">
															...
															${(bgypApplyVo.applyCheckState eq 2 or bgypApplyVo.applyCheckState eq 1)?'disabled="disabled"':''}
															 value="${bgypApplyVo.id}"/> 
												...
												<td>
													<c:choose>
														<c:when test="${bgypApplyVo.applyCheckState eq 3}">${bgypApplyVo.spContent}</c:when>
														<c:otherwise>
															--
														</c:otherwise>
													</c:choose>
												...
												<td>
													<c:choose>
														<c:when test="${bgypApplyVo.applyCheckState eq 0 or bgypApplyVo.applyCheckState eq 3 }">
															<a href="javascript:void(0)" busId="${bgypApplyVo.id}" class="updateBgypApply">编辑</a>
															|
															<a href="javascript:void(0)" busId="${bgypApplyVo.id}" class="spCheckBgypApply">查看</a>
														</c:when>
														<c:when test="${bgypApplyVo.applyCheckState eq 1 or bgypApplyVo.applyCheckState eq 2}">
															<a href="javascript:void(0)" busId="${bgypApplyVo.id}" class="spCheckBgypApply">查看</a>
														</c:when>
													</c:choose>
												</td>
							...
							<tags:pageBar url="/bgypApply/listPagedBgypApply"></tags:pageBar>
						</div>
						</c:when>
						<c:otherwise>
