				...
				<div class="col-md-12 col-xs-12 ">
					<div class="widget">
						<div class="widget-header ps-titleHeader bordered-bottom bordered-themeprimary">
							<div>
								<form action="/financial/writeOff/listWriteOffOfAuth" id="searchForm" class="subform">
									...
									<div class="btn-group pull-left searchCond">
												...
												<a class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" title="排序">
													<c:choose>
														<c:when test="${not empty writeOff.orderBy}">
															...
															<c:if test="${writeOff.orderBy=='executor'}">按审批人</c:if> 
															<c:if test="${writeOff.orderBy=='crTimeNewest'}">按创建时间(降序)</c:if> 
															<c:if test="${writeOff.orderBy=='crTimeOldest'}">按创建时间(升序)</c:if>
															</font>
														</c:when>
										...
										<div class="table-toolbar ps-margin">
											<div class="btn-group">
												<a class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" title="申请人筛选">
													<c:choose>
														<c:when test="${not empty writeOff.creator}">
															<font style="font-weight:bold;">${writeOff.creatorName}</font>
														</c:when>
										...
										<div style="float: left;width: 250px;display: none">
											<select list="listCreator" listkey="id" listvalue="userName" id="creator_select" name="listCreator.id" multiple="multiple" moreselect="true" style="width: 100%; height: 100px;">
												<c:forEach items="${writeOff.listCreator }" var="obj" varStatus="vs">
								...
								<div class="ps-margin ps-search searchCond">
									<span class="input-icon">
										<input id="searchFlowName" name="flowName" value="${writeOff.flowName}" class="form-control ps-input formElementSearch" type="text" placeholder="请输入关键字">
							...
							<div class=" padding-top-10 text-left " style="display:${empty writeOff.listCreator ? 'none':'block'}">
								<strong>申请人筛选:</strong>
								<c:forEach items="${writeOff.listCreator }" var="obj" varStatus="vs">
						...
						<c:choose>
						<c:when test="${not empty listWriteOffOfAuth}">
						<div class="widget-body">
							<form action="/workFlow/delSpFlow?sid=${param.sid}" method="post"
								...
								<table class="table table-striped table-hover" id="editabledatatable">
									...
									<tbody>
										<c:forEach items="${listWriteOffOfAuth}" var="writeOffVo" varStatus="vs">
											<tr class="optTr">
							...
							<tags:pageBar url="/financial/writeOff/listWriteOffOfAuth"></tags:pageBar>
						</div>
						</c:when>
						<c:otherwise>
