...
<div class="widget-header ps-titleHeader bordered-bottom bordered-themeprimary">
	...
	<div>
	<form action="/financial/listLoanOffsPage" class="subform" id="searchForm">
		...
		<input type="hidden" name="sendNotice" id="sendNotice" value="${loanOff.sendNotice}">
		<input type="hidden" name="isBusinessTrip" id="isBusinessTrip" value="${loanOff.isBusinessTrip}">
		<input type="hidden" name="balanceState" id="balanceState" value="${loanOff.balanceState}">
				...
				<div class="table-toolbar ps-margin">
					<div class="btn-group">
						<a class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" title="报销类型">
							<c:choose>
                                <c:when test="${not empty loanOff.isBusinessTrip}">
                                    <font style="font-weight:bold;">
                                        <c:choose>
                                            <c:when test="${loanOff.isBusinessTrip==1}">出差报销</c:when>
                                        </c:choose>
                                        <c:choose>
                                            <c:when test="${loanOff.isBusinessTrip==0}">一般报销</c:when>
                                        </c:choose>
                                    </font>
                                </c:when>
                                <c:otherwise>报销类型</c:otherwise>
                            </c:choose> 
							<i class="fa fa-angle-down"></i>
						</a>
						<ul class="dropdown-menu dropdown-default">
				...
				<div class="table-toolbar ps-margin">
					<div class="btn-group">
						<a class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" title="通知状态">
							<c:choose>
                                <c:when test="${not empty loanOff.sendNotice}">
                                    <font style="font-weight:bold;">
                                        <c:choose>
                                            <c:when test="${loanOff.sendNotice==1}">已通知</c:when>
                                        </c:choose>
                                        <c:choose>
                                            <c:when test="${loanOff.sendNotice==0}">未通知</c:when>
                                        </c:choose>
                                    </font>
                                </c:when>
				...
				<div class="table-toolbar ps-margin">
					<div class="btn-group">
						<a class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" title="报销状态">
							<c:choose>
                                <c:when test="${not empty loanOff.balanceState}">
                                    <font style="font-weight:bold;">
                                    	<c:choose>
                                            <c:when test="${loanOff.balanceState==2}">审批中</c:when>
                                        </c:choose>
                                        <c:choose>
                                            <c:when test="${loanOff.balanceState==1}">已结算</c:when>
                                        </c:choose>
                                        <c:choose>
                                            <c:when test="${loanOff.balanceState==0}">待结算</c:when>
                                        </c:choose>
                                        <c:choose>
                                            <c:when test="${loanOff.balanceState==3}">待办理</c:when>
                                        </c:choose>
                                    </font>
                                </c:when>
				...
				<div class="table-toolbar ps-margin">
					<div class="btn-group cond" id="moreCondition_Div">
						<a class="btn btn-default dropdown-toggle btn-xs" onclick="displayMoreCond('moreCondition_Div')">
							<c:choose>
								<c:when test="${not empty loanOff.startDate || not empty loanOff.endDate}">
						...
						<div class="dropdown-menu dropdown-default padding-bottom-10 subMore" style="min-width: 330px;">
							<div class="ps-margin ps-search padding-left-10">
								...
								<input class="form-control dpd2 no-padding condDate" type="text" readonly="readonly" id="startDate" name="startDate" placeholder="开始时间"
									onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate: '#F{$dp.$D(\'endDate\',{d:-0});}'})" value="${loanOff.startDate}"/>
								<span>~</span>
		...
		<div class="ps-margin ps-search searchCond">
			<span class="input-icon">
				<input name="loanOffName" class="form-control ps-input moreSearch" type="text" placeholder="请输入关键字" value="${loanOff.loanOffName }">
		...
		<div class="ps-clear" id="formTempData">
			<select list="listCreator" listkey="id" listvalue="userName" id="applyUser_select" 
				...
				<c:forEach items="${loanOff.listCreator }" var="user" varStatus="">
					<option value="${user.id}">${user.userName }</option>
				</c:forEach>
			</select>
		</div>
		<!-- 筛选条件显示 -->
		<div class="padding-top-5 padding-bottom-5 text-left moreUserListShow" style="display:${fn:length(loanOff.listCreator)>0?'block':'none'}" id="applyUser_selectDiv">
			<strong>申请人:</strong>
				<c:forEach items="${loanOff.listCreator }" var="user" varStatus="">
...
<c:choose>
	<c:when test="${not empty listLoanOffs}">
	 ...
	 <div class="wrapper"  style="min-height: 135px;background-color: #FFFFFF;">
       <div class="content-wrapper">
		 ...
		 <c:forEach items="${listLoanOffs}" var="obj" varStatus="vs">
			<div class="content-list-item" onmouseover="showCheck('${obj.id}')" onmouseout="hideCheck('${obj.id}')">
					<c:choose>
						<c:when test="${ obj.flowState == 4 && obj.balanceState != 1}">
                    ...
                    <div class="description" style="width: 40%">
                        <p class="type">${obj.isBusinessTrip == 1?"出差报销":"一般报销" }</p>
                        <p><a href="javascript:void(0)" class="spFlowIns" insId="${obj.instanceId }" style="color: #2a6496" title="${obj.loanOffName }">${obj.loanOffName }</a></p>
                    </div>
                    <c:choose>
						<c:when test="${ obj.flowState == 4 && obj.balanceState != 1}">
                    ...
                    <c:choose>
                    	<c:when test="${obj.balanceState == 1}">
                   		...
                   		<c:when test="${obj.flowState == 4 && obj.balanceState != 1}">
                   			 <div class="operation">
		                     	  <a href="javascript:void(0)" onclick="addNotice('${obj.id}','${obj.loanOffItemFee}','${obj.creatorName }','${obj.creator }','1','${obj.id }','${obj.loanOffName }','${obj.instanceId }')" >
									<c:choose>
								  ...
								  <a href="javascript:void(0)" onclick="addBanlance('${obj.id}','${obj.loanOffItemFee}','${obj.creatorName }','${obj.creator }','031','${obj.id }','${obj.instanceId }')" >完成结算</a>
		                  	</div>
                   		</c:when>
                   		<c:otherwise>
                   			<div class="operation">
		...
		<tags:pageBar url="/financial/listLoanOffsPage"></tags:pageBar>
	  </div>
