...
<body>
											...
											<tr class="optTr" taskID="${taskVo.id}" style="clear: both">
												<td class="optTd" style="height: 47px">
													<label class="optCheckBox" style="display: none;width: 20px">
														<input class="colored-blue" type="checkbox" name="ids" value="${taskVo.id}"
														${(taskVo.owner==userInfo.id) ? '':'disabled="disabled"' } state="${taskVo.state }" />
														<span class="text"></span>
													</label> 
													<label class="optRowNum" style="display: block;width: 20px">${vs.count}</label></td>
												<td>
												...
												<td class="center">
													<c:choose>
														<c:when test="${taskVo.state==3 || taskVo.state==4}">--</c:when>
														<c:otherwise>${taskVo.dealTimeLimit}</c:otherwise>
												</c:choose>
												</td>
							 							...
							 							--
															...
															<c:otherwise>
																<div class="ticket-user pull-left other-user-box" data-container="body"
																	...
																	<img class="user-avatar" title="${taskVo.executorName}" 
												...
												<td class="no-padding-left no-padding-right" align="center">
													<div class="ticket-user pull-left other-user-box">
														<img class="user-avatar" title="${taskVo.ownerName}" 
															src="/downLoad/userImg/${userInfo.comId}/${taskVo.owner}"/>
	...
	$(function(){
		...
		$("#searchForm").ajaxSubmit({
		     	type:"POST",
		         url:"/task/ajaxAllTaskCount?entry=0&t="+Math.random(),
		         dataType:"json",
		         data:selectData,
		         beforeSend: function(){
		         ...
		         success:function(data){
					...
					if(data.countType == 1){
						...
						countTaskGrade(data.allTaskCount);
					}
		         },
		...
		$("#subDiv").on("click","a",function(){
			...
			$("#searchForm").find(".condDate").val('');
			if(executor){
				$("#searchForm").find("input[name='state']").attr("value",1);
				var option = $("<option></option>");
				$(option).attr("value",executor);
				$(option).html(userName);
				$("#executor_select").html($(option));
				
				$("#operator_select").html("");
			}else if(operatorNotExecutor){
				$("#searchForm").find("#containExecutor").remove();
				var input = $('<input type="hidden" name="containExecutor" value="1" id="containExecutor">');
				$("#searchForm").append($(input));
				$("#searchForm").find("input[name='state']").attr("value",'');
				var option = $("<option></option>");
				$(option).attr("value",operatorNotExecutor);
				$(option).html(userName);
				$("#operator_select").html($(option));
