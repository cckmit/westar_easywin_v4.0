                                            ...
                                            <c:choose>
												<c:when
													...
													<input class="colorpicker-default form-control"
														...
														id="dealTimeLimit"
														preval="${fn:substring(task.dealTimeLimit,0,10) }"
														name="dealTimeLimit" type="text"
														value="${fn:substring(task.dealTimeLimit,0,10) }">
												</c:when>
												<c:otherwise>
														${fn:substring(task.dealTimeLimit,0,10) }
												</c:otherwise>
											</c:choose>
                ...
                <div class="widget-body bordered-radius">
                    <div class="tickets-container tickets-bg tickets-pd">
                        <ul class="tickets-list">
                            <li class="ticket-item no-shadow clearfix ticket-normal">
                                <div class="pull-left task-left-text task-height">
									${task.busType=='005'?'关联项目':task.busType=='012'?'关联客户':task.busType=='022'?'关联审批':'关联模块'}：
                                </div>
                                <div class="pull-left col-lg-7 col-sm-7 col-xs-7">
                                    <div class="row">
                                        <c:choose>
    ...
    <script type="text/javascript">
	    ...
	    var taskOptForm = {
	    		taskOptMenu:function(){
	    			...
	    			//添加办结按钮
	    			taskOptForm.addTaskFinishMenu(taskObj.state,taskObj.ownState);
	    			//添加更多按钮
	    		...
	    		},addTaskFinishMenu:function(state,ownState){
	    			if(ownState=='1'){//任务办理中，且当前人员是发起人员，则添加办结
	    				var _finishA = $('<a href="javascript:void(0)" class="red finishTaskBtn" title="结束任务"></a>');
	    				$(_finishA).html('<i class="fa fa-power-off"></i>办结');
