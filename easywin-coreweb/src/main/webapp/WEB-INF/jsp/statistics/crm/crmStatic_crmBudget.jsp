...
<html xmlns="http://www.w3.org/1999/xhtml">
                                ...
                                <div>
										...
										<div class="searchCond" style="display: block">
											...
											<div class="widget-buttons ps-widget-buttons">
												...
												<button class="btn btn-info  btn-xs" style="display:none" id ="excelExport" onclick="excelExport('资金预算统计表')" >导出excel</button>
											</div>
                               		 </div>
	...
	<script type="text/javascript">
	    ...
	    var url = '/statistics/crm/listCrmStatisByType?sid=${param.sid}&statisticsType=7';
	    //缓存数据用户穿透
	    var caseData = {};
	...
	function getRandom(min,max){
    	   ...
    	   $.each(listBusReport,function(index,obj){
    		    //图例
	      		...
	      		caseData[obj.name]=obj.type
	      		 $.each(obj.childModStaticVo,function(xindex,vo){
	      		 	crmNumArray.push(vo.value)
      		 	 });
    	   });
       ...
       myChart.on("click",function(params){
    	    ...
    	    var stage = caseData[params.name];
			var owner = $("#owner").val();
			var areaIdAndType = $("#areaIdAndType").val();
			var startDate = $("#startDate").val();
			var endDate = $("#endDate").val();
    	    ...
    	    var otherUrl = "/crm/customerListPage?sid=${param.sid}&searchTab=16";
       ...
       function initTable(data){
		   ...
		   headTh = $("<th style='text-align:center;font-size:13px;width:15;font-weight:bold;'>资金总额</th>");
    	   $(headTr).append(headTh)
    	     headTh = $("<th style='text-align:center;font-size:13px;width:15;font-weight:bold;'>客户数量</th>");
    	   $(headTr).append(headTh)
    	   $(thead).append(headTr);
    	   $(table).append(thead)
    	   ...
    	   var tbody = $("<tbody></tbody>");
	    	   $.each(listBusReport,function(index,obj){
	    		   var bodyTr = $(tbody).find("tr[rownum='"+index+"']")
	    		   if(bodyTr.length == 0){
	    			   bodyTr = $("<tr rownum='"+index+"'></tr>");
	    			   $(tbody).append(bodyTr);
	    			   bodyTr = $(tbody).find("tr[rownum='"+index+"']")
	    		   }
	    		   ...
	    		   $(bodyTr).append(valueTd);
	    		     $.each(obj.childModStaticVo,function(xindex,vo){
	    		     		var numTd =$("<td style='font-size:12px;color:rgb(0,91,255);text-align:center;'><a href='javascript:void(0)' typeId='"+vo.type+"'>"+vo.value+"</a></td>");
	    		     	   $(bodyTr).append(numTd);
	    		     	   $(numTd).attr("cellNum", 1);
	    		     })
	    		     $(valueTd).attr("cellNum", 0);
	    	   $(table).append(tbody)
	       })
