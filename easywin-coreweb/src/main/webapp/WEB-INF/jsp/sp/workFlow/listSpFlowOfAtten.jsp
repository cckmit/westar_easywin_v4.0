				...
				<div class="col-md-12 col-xs-12 ">
					<div class="widget">
						<div class="widget-header ps-titleHeader bordered-bottom bordered-themeprimary">
							<div>
								<form action="/workFlow/listSpFlowOfAtten" id="searchForm" class="subform">
												...
												<c:choose>
													<c:when test="${not empty listHourlyUser}">
														<c:forEach items="${listHourlyUser}" var="owner" varStatus="vs">
															<li>
																<a href="javascript:void(0)" onclick="userOneForUserIdCallBack(
															'${owner.id}','executor','${owner.userName}','');">${owner.userName}</a>
															</li>
														</c:forEach>
													</c:when>
												</c:choose> --%>
						...
						<c:choose>
							<c:when test="${not empty listSp}">
								<div class="widget-body">
											...
											<c:forEach items="${listSp}" var="sp" varStatus="vs">
												<tr class="optTr">
													<td style="text-align:center;" class="optTd"><label class="optCheckBox" style="display: none;width: 20px">
															<input class="colored-blue" type="checkbox" name="ids" value="${sp.id}" ${(sp.creator==userInfo.id)?'':'disabled="disabled"' } state="${sp.flowState }" />
									...
									<tags:pageBar url="/workFlow/listSpFlowOfAtten"></tags:pageBar>
								</div>
							</c:when>
							<c:otherwise>
