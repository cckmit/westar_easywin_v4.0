				...
				<div class="col-md-12 col-xs-12 ">
					<div class="widget">
						...
						<div class="widget-header ps-titleHeader bordered-bottom bordered-themeprimary">
							...
							<div>
							<form id="searchForm" action="/meeting/listPagedTodoMeeting" class="subform">
								<input type="hidden" name="searchTab" id="searchTab" value="${empty param.searchTab?'11':param.searchTab}"> <input type="hidden" name="sid" value="${param.sid}" /> <input
									type="hidden" value="${meeting.organiser}" name="organiser" id="organiser"> <input type="hidden" value="${meeting.organiserName}" name="organiserName" id="organiserName">
													...
													<%-- <c:choose>
														<c:when test="${not empty listOwners}">
															...
															<c:forEach items="${listOwners}" var="owner" varStatus="vs">
																<li><a href="javascript:void(0)" onclick="userOneForUserIdCallBack('${owner.id}','organiser','${owner.userName}','organiserName');">${owner.userName}</a></li>
															</c:forEach>
														</c:when>
													</c:choose> --%>
												</ul>
						...
						<c:choose>
							<c:when test="${not empty listMeetings}">
								<div class="widget-body">
												...
												<c:forEach items="${listMeetings}" var="obj" varStatus="vs">
													<tr class="optTr">
														...
														<td valign="middle"><c:choose>
																<c:when test="${obj.timeOut==2 && userInfo.id==obj.organiser}">
																	<!-- 会议已发布，未开始 -->
																	<a href="javascript:void(0)" onclick="editMeeting('${obj.id}','${obj.meetingType}',${obj.meetingState})" style="color: blue">编辑</a>| 已参会
															</c:when>
																<c:when test="${obj.timeOut==1 && userInfo.id==obj.organiser}">
																	<!-- 会议已发布，已开始 -->
																	<a href="javascript:void(0)" onclick="invMeetUser('${obj.id}')" style="color: blue">编辑</a>| 已参会
															</c:when>
																<c:otherwise>
																	<c:choose>
																		<c:when test="${obj.checkState==0}">
																			<!--未确认 -->
																			<a href="javascript:void(0)" onclick="checkMeeting('${obj.id}',1)" style="color:blue">参会</a>|
																		<a href="javascript:void(0)" onclick="checkMeeting('${obj.id}',2)" style="color:red">拒绝</a>
																		</c:when>
																		<c:when test="${obj.checkState==1}">
																			<a href="javascript:void(0)" onclick="viewMeeting('${obj.id}')" style="color:#008000;">查看</a>|
																		已参会
																		</c:when>
																		<c:when test="${obj.checkState==2}">
																			<a href="javascript:void(0)" onclick="viewMeeting('${obj.id}')" style="color:#008000;">查看</a>|
																		已拒绝
																		</c:when>
																	</c:choose>
									...
									<tags:pageBar url="/meeting/listPagedTodoMeeting"></tags:pageBar>
								</div>
							</c:when>
							<c:otherwise>
